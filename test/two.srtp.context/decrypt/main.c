...
int main(int argc, char *argv[])
{
    ...
    for(;;)
    {
        ...
        length = datalen;
        if(flag){
            status = srtp_unprotect(srtp_ctx, buf, &length);
        }else{
            status = srtp_unprotect_rtcp(srtp_ctx, buf, &length);
        }
        
        if (status) {
            printf("unprotect error:%d, srclen:%d, style:%s\n",
                    status, datalen, flag ? "rtp":"rtcp");
        }else{
            printf("unprotect succe, srclen:%d, dstlen:%d, style:%s\n",
